...
public interface MarginSummaryRepository extends JpaRepository<MarginSummary, MarginSummaryId> {
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM MarginSummary m " +
            "WHERE m.id.quoteNumber = ?1 " +
            "AND m.id.type = ?2 " +
            "AND m.id.modelCode = ?3 " +
            "AND m.id.series = ?4 " +
            ...
            "AND m.id.userId = ?6 " +
            "AND m.id.region = ?7 ")
    void deleteMarginSummary(String quoteNumber, int type, String modelCode, String series,
                             String currency, int userId, String region);
}
