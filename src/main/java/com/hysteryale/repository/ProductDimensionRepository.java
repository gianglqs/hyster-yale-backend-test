...
public interface ProductDimensionRepository extends JpaRepository<Product, String> {
    ...
    @Query(value = "SELECT p.plant FROM product p WHERE p.metaseries = :metaSeries LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM product p WHERE p.metaseries = :metaSeries LIMIT 1", nativeQuery = true)
